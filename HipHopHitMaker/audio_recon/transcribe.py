...
# Ramo parallelo: riconoscimento genere musicale (NON Canary)
from genre import GenreClassifier, format_genre_block
# Ramo parallelo: note più usate + tonalità (NON Canary)
from notes import NoteDetector, format_notes_block
# Ramo parallelo: BPM + tempo (NON Canary)
...
def transcribe_asr(model: SALM, samples: np.ndarray) -> str:
    ...
    Approccio: salva WAV temporaneo → model.generate(prompts=[...])
    API NeMo 2.x SALM. NeMo gestisce internamente PCM → log-mel
    → embedding → testo. Output = token IDs → ids_to_text().
    """
    # Scrivi WAV temporaneo (SALM vuole un path audio dentro il prompt)
    ...
    try:
        ...
        # API NeMo 2.x SALM: generate() con audio dentro il messaggio.
        # audio_locator_tag = segnaposto interno (<|audioplaceholder|>)
...
def query_llm_mode(model: SALM, transcript: str, user_prompt: str) -> str:
    ...
    # disable_adapter() spegne il LoRA audio → Qwen3 ragiona solo sul testo
...
def print_file_result(filepath, audio_info, duration_sec,
                      ...
                      genre_result=None, notes_result=None, tempo_result=None):
...
def parse_args():
    ...
    p.add_argument("--genre", action="store_true",
                   help="Attiva riconoscimento genere musicale (ramo parallelo all'ASR)")
    p.add_argument("--genre-model", default="dima806/music_genres_classification",
                   help="Checkpoint HF audio-classification per il genere")
    p.add_argument("--notes", action="store_true",
                   help="Attiva analisi note più usate + tonalità (ramo parallelo)")
    p.add_argument("--tempo", action="store_true",
                   help="Attiva analisi BPM + tempo (ramo parallelo)")
    return p.parse_args()
...
def main():
    ...
    # Carica il classificatore di genere una volta sola (se richiesto)
    genre_clf = None
    if args.genre:
    ...
    # Inizializza il rilevatore note una volta sola (se richiesto)
    note_det = None
    if args.notes:
        note_det = NoteDetector()
        print("  Note detector OK")

    # Inizializza il rilevatore tempo una volta sola (se richiesto)
    tempo_det = None
    if args.tempo:
    ...
    for filepath in args.files:
        ...
        try:
            ...
            # 1b. RAMO GENERE — sul buffer GREZZO, prima del preprocess ASR
            genre_result = None
            if genre_clf:
                print("    Genre branch...")
            ...
            # 1c. RAMO NOTE — anch'esso sul buffer GREZZO
            notes_result = None
            if note_det:
                print("    Notes branch...")
            ...
            # 1d. RAMO TEMPO — anch'esso sul buffer GREZZO
            tempo_result = None
            if tempo_det:
                print("    Tempo branch...")
            ...
            print_file_result(
                ...
                time.time() - t0, genre_result, notes_result, tempo_result,
            )
            successful += 1
